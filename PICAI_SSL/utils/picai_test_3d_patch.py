...
def var_all_case_LA_mean(model1, model2, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    image_list = ["/content/drive/MyDrive/SemiSL/Dataset/PICAI_dataset/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
...
def test_single_case(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    score_map = score_map/np.expand_dims(cnt,axis=0)
    # label_map = (score_map[0]>0.5).astype(np.int)
    label_map = (score_map[0] > 0.5).astype(int)  # Recommended
    if add_pad:
        label_map = label_map[wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
        score_map = score_map[:,wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
    return label_map, score_map
...
def var_all_case_LA_plus(model_l, model_r, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    ...
    with open('/content/drive/MyDrive/SemiSL/Code/SSL_Project/PICAI_SSL/Datasets/picai/data_split/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = ["/content/drive/MyDrive/SemiSL/Dataset/PICAI_dataset/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
...
def test_all_case_plus(model_l, model_r, image_list, num_classes, patch_size=(256, 256, 16), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
